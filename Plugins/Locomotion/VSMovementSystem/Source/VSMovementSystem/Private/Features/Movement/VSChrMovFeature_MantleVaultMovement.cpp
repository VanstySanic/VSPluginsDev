...
void UVSChrMovFeature_MantleVaultMovement::GetLifetimeReplicatedProps(TArray<class FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_WITH_PARAMS_FAST(UVSChrMovFeature_MantleVaultMovement, ReplicatedSnappedParams, SharedParams);
}
...
void UVSChrMovFeature_MantleVaultMovement::TryMantleVault(const TArray<FDataTableRowHandle>& SettingRows, TEnumAsByte<EVSMantleVaultMovementType::Type> SupportedMovementType)
{
	...
	{
		TryMantleVault_Server(SettingRows, SupportedMovementType);
	}
	else
	{
...
void UVSChrMovFeature_MantleVaultMovement::StopMantleVault()
{
	...
	ReplicatedSnappedParams = FVSMantleVaultSnappedParams();
}
...
void UVSChrMovFeature_MantleVaultMovement::UpdateMovement_Implementation(float DeltaTime)
{
	...
	GetCharacterMovement()->SafeMoveUpdatedComponent(TargetLocationWS - GetCharacter()->GetActorLocation(), GetCharacter()->GetActorRotation(), false, HitResult);
	...
	// TODO Might be changed in future versions.
...
void UVSChrMovFeature_MantleVaultMovement::TryMantleVaultInternal(const TArray<FDataTableRowHandle>& SettingRows, TEnumAsByte<EVSMantleVaultMovementType::Type> SupportedMovementType)
{
	if (SupportedMovementType == EVSMantleVaultMovementType::None || (SettingRows.IsEmpty() && DefaultSettingRows.IsEmpty())) return;
...
void UVSChrMovFeature_MantleVaultMovement::MantleVaultBySnappedParams(const FVSMantleVaultSnappedParams& SnappedParams)
{
	...
	UVSGameplayLibrary::SweepSingleByShapeAndChannels(this, ComponentTraceHit, SnappedParams.StartFrontWallPointWS, SnappedParams.StartFrontWallPointWS + SnappedParams.StartMovementDirection2DWS * 16.f, FQuat::Identity,
	...
	if (GetOwnerActor()->HasAuthority())
	{
		...
		MARK_PROPERTY_DIRTY_FROM_NAME(UVSChrMovFeature_MantleVaultMovement, ReplicatedSnappedParams, this);
	}
...
bool UVSChrMovFeature_MantleVaultMovement::CalcMantleVaultSnappedParams(FVSMantleVaultSnappedParams& OutSnappedParams, const FDataTableRowHandle& SettingsRow, TEnumAsByte<EVSMantleVaultMovementType::Type> SupportedMovementType)
{
	...
	/** SC means scaled. */
	const float CapsuleHalfHeightSC = GetCharacter()->GetCapsuleComponent()->GetScaledCapsuleHalfHeight();
	...
	for (float i = PlatformHeightRangeSC.X; i < PlatformHeightRangeSC.Y + CapsuleRadiusSC; i += CapsuleRadiusSC)
	{
		...
		for (const FDataTableRowHandle& AvailableAnimRow : AvailableAnimRows)
		{
			...
			const FVector& FarthestVerticalBlockTraceStart = (HorizontalSpaceHitResult.bBlockingHit ? HorizontalSpaceHitResult.Location : HorizontalSpaceHitResult.TraceEnd) + 0.f;
			const FVector& FarthestVerticalBlockTraceEnd = FarthestVerticalBlockTraceStart - CapsuleRadiusSC * GetUpDirection() - VerticalCollisionOffset - 0.01f * GetUpDirection();
			...
			const float ReachTargetWallDistanceToWall2DMin = FMath::Min(AnimReachTargetWallDistanceToWall2DMinByHorizontalSpace, AnimReachTargetWallDistanceToWall2DMinByWallThickness);
			...
			const FTransform& ComponentTransform = FrontWallTraceHitResult.Component->GetComponentTransform();
			OutSnappedParams.SettingsRow = SettingsRow;
			OutSnappedParams.AnimSettingsRow = AvailableAnimRow;
			OutSnappedParams.StartMovementDirection2DWS = MovementDirection2DWS;
			OutSnappedParams.StartFrontWallPointWS = FrontWallPointWS;
			OutSnappedParams.MovementDirection2DRS = ComponentTransform.InverseTransformVectorNoScale(MovementDirection2DWS);
			OutSnappedParams.FrontWallPointRS = ComponentTransform.InverseTransformPosition(FrontWallPointWS);
			OutSnappedParams.GroundPivotPointRS = ComponentTransform.InverseTransformPosition(GroundPivotVerticalTraceHitResult.ImpactPoint + VerticalCollisionOffset);
...
void UVSChrMovFeature_MantleVaultMovement::TryMantleVault_Server_Implementation(const TArray<FDataTableRowHandle>& SettingRows, EVSMantleVaultMovementType::Type SupportedMovementType)
{
	TryMantleVaultInternal(SettingRows, SupportedMovementType);
}

void UVSChrMovFeature_MantleVaultMovement::OnRep_ReplicatedSnappedParams()
{
	bool bShouldMantleVault = true;
