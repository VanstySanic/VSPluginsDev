...
void UVSInteractiveFeatureAgent::OnFeatureDeactivated_Implementation()
{
...
{
	if (!bAllowInspection || !IsActive() || !SourceAgent || SourceAgent->IsInteracting()) return false;
...
{
	if (!bAllowInteraction || !IsActive() || !SourceAgent || GetAvailableActionFeatures(SourceAgent).IsEmpty()) return false;
	FGameplayTagContainer SourceTags = SourceAgent->GetGameplayTagController()->GetGameplayTags();
	return true;
}
