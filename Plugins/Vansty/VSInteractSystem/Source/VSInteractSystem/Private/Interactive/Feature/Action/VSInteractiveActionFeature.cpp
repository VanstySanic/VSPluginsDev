...
{
	if (!bAllowInteraction || !IsActive() || !SourceAgent || SourceAgent->IsInteracting()) return false;
...
void UVSInteractiveActionFeature::OnFeatureDeactivated_Implementation()
{
	TMap<TWeakObjectPtr<UVSInteractFeatureAgent>, float> CopiedInteractingSourceTimeMap = InteractingSourceTimeMap;
	for (const TPair<TWeakObjectPtr<UVSInteractFeatureAgent>, float>& SourceTimeMap : CopiedInteractingSourceTimeMap)
	{
		EndInteraction(SourceTimeMap.Key.Get(), true);
	} 
