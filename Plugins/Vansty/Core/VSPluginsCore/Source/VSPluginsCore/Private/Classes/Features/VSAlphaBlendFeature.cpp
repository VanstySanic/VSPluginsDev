...
void UVSAlphaBlendFeature::Uninitialize_Implementation()
{
	SetAlpha(DefaultAlpha);
...
void UVSAlphaBlendFeature::EndPlay_Implementation()
{
	SetAutoUpdate(false);
...
void UVSAlphaBlendFeature::SetTime(float InTime)
{
	...
	OnUpdated_Native.Broadcast(this, Alpha, UpdatedTime);
...
void UVSAlphaBlendFeature::SetUpdateTimeInterval(float Interval)
{
	if (TickProxy)
	{
		TickProxy->PrimaryObjectTick.TickInterval = Interval;
	}
}
...
void UVSAlphaBlendFeature::DoFinishedLogic()
{
	...
	OnFinished_Native.Broadcast(this, Alpha, UpdatedTime);
...
UVSAlphaBlendProxy* UVSAlphaBlendProxy::CreateAlphaBlendProxy(UObject* Outer)
{
	if (!Outer || !Outer->GetWorld()) return nullptr;
	UVSAlphaBlendProxy* Proxy = NewObject<UVSAlphaBlendProxy>(Outer);
