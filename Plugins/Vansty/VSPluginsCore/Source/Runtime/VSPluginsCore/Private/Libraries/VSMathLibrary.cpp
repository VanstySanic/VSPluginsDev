...
int32 UVSMathLibrary::GetDecimalPlacesTrimmedFloat(float Value, int32 MaxDecimals)
{
	/** [Generated by ChatGPT] */
	
	if (!FMath::IsFinite(Value) || MaxDecimals <= 0)
	{
		return 0;
	}

	// 1) Round to the desired decimal precision to wipe binary float artifacts.
	const double Scale = FMath::Pow(10.f, MaxDecimals);
	const double Rounded = FMath::RoundHalfToZero(Value * Scale) / Scale; // symmetric rounding around 0

	// 2) Format with fixed decimals, then strip trailing zeros.
	FString Str = FString::Printf(TEXT("%.*f"), MaxDecimals, Rounded);

	// Optional: trim a possible minus sign for easier processing (doesn't affect decimal places).
	int32 StartIdx = (Str.StartsWith(TEXT("-"))) ? 1 : 0;

	// 3) Find decimal point.
	int32 PointIndex = INDEX_NONE;
	for (int32 i = StartIdx; i < Str.Len(); ++i)
	{
		if (Str[i] == TCHAR('.'))
		{
			PointIndex = i;
	...
	// 4) Strip trailing zeros in the fractional part.
	int32 End = Str.Len() - 1;
	while (End > PointIndex && Str[End] == TCHAR('0'))
	{
		--End;
	}

	// If decimal point becomes the last character, there is effectively no fraction.
	if (End == PointIndex)
	{
		return 0;
	}
