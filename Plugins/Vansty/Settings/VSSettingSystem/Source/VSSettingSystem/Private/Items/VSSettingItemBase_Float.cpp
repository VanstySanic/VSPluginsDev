...
void UVSSettingItemBase_Float::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	else if (PropertyChangedEvent.GetMemberPropertyName() == GET_MEMBER_NAME_CHECKED(UVSSettingItemBase_Float, NamedValues))
	{
...
void UVSSettingItemBase_Float::UnbindWidgetInternal_Implementation(UWidget* Widget, FName TypeName)
{
	if (TypeName == FName("Value"))
	{
		if (UComboBoxString* ComboBox = Cast<UComboBoxString>(Widget))
		{
			if (UVSGameplayLibrary::IsInGame())
			{
				ComboBox->OnSelectionChanged.RemoveDynamic(this, &UVSSettingItemBase_Float::OnValueComboBoxStringSelectionChanged);
			}
		}
		else if (UCommonRotator* Rotator = Cast<UCommonRotator>(Widget))
		{
			if (UVSGameplayLibrary::IsInGame())
			{
				Rotator->OnRotatedWithDirection.RemoveDynamic(this, &UVSSettingItemBase_Float::OnValueRotatorRotatedWithDirection);
			}
		}
		else if (USlider* Slider = Cast<USlider>(Widget))
		{
...
TArray<float> UVSSettingItemBase_Float::CalcGeneratedOptions_Implementation() const
{
	...
	{
		for (float i = DesiredValueRange.X; i <= DesiredValueRange.Y + DesiredValueStep - 1.f; i += DesiredValueStep)
		{
			Options.Add(FMath::Clamp(i, DesiredValueRange.X, DesiredValueRange.Y));
		}
	}
	else
	{
		for (float i = DesiredValueRange.Y; i >= DesiredValueRange.X - DesiredValueStep + 1.f; i -= DesiredValueStep)
		{
			Options.Add(FMath::Clamp(i, DesiredValueRange.X, DesiredValueRange.Y));
		}
	}
...
{
	const float ConditionedSettingValue = GetSettingToConditionValue(GetValue(EVSSettingItemValueSource::Settings));
	if (FMath::IsNearlyEqual(ConditionedSettingValue, NewValue, DesiredValueStep / 2.f)) return;
	...
	for (UWidget* Widget : GetBoundWidgetsOfType(FName("Value")))
	{
		if (USlider* Slider = Cast<USlider>(Widget))
		{
			...
			if (!Slider->HasMouseCapture())
			{
				...
				{
					const int32 SliderValueNearestIndex = UVSSetContainerLibrary::GetArrayNearestElementIndex(SliderValue, Options);
					/** The value set is within one step. */
					if (FMath::Abs(SliderValueNearestIndex - SettingOptionIndex) <= 1)
					{
						/** Snap the value to option. */
						const int32 NewValueIndex = SettingOptionIndex + (NewValue > ConditionedSettingValue ? 1 : -1) * (bOptionsHighToLow ? -1 : 1);
