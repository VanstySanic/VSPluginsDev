...
void UVSSettingItemBase_Integer::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	else if (PropertyChangedEvent.GetMemberPropertyName() == GET_MEMBER_NAME_CHECKED(UVSSettingItemBase_Integer, NamedValues))
	{
...
void UVSSettingItemBase_Integer::BindWidgetInternal_Implementation(UWidget* Widget, FName TypeName)
{
	...
	{
		/** Generate options. */
		TArray<int32> Options = GetOptions();
...
FText UVSSettingItemBase_Integer::GetValueContentText_Implementation(int32 InValue) const
{
...
{
	const int32 ConditionedSettingValue = GetConditionToSettingValue(GetValue(EVSSettingItemValueSource::Settings));
	const int32 NewIntValue = FMath::RoundToInt(NewValue);

	if (ConditionedSettingValue == NewIntValue) return;
			...
			if (!Slider->HasMouseCapture())
			{
				...
				if (SliderValue == NewIntValue)
				{
					...
					if (FMath::Abs(SliderValueNearestIndex - SettingOptionIndex) <= 1)
					{
						...
						const int32 NewValueIndex = SettingOptionIndex + (NewIntValue > ConditionedSettingValue ? 1 : -1) * (bOptionsHighToLow ? -1 : 1);
